...
import pey.vpn.net.AppConfig.DEFAULT_NETWORK
import pey.vpn.net.AppConfig.DNS_ALIDNS_ADDRESSES
import pey.vpn.net.AppConfig.DNS_ALIDNS_DOMAIN
import pey.vpn.net.AppConfig.DNS_CLOUDFLARE_ADDRESSES
import pey.vpn.net.AppConfig.DNS_CLOUDFLARE_DOMAIN
import pey.vpn.net.AppConfig.DNS_DNSPOD_ADDRESSES
import pey.vpn.net.AppConfig.DNS_DNSPOD_DOMAIN
import pey.vpn.net.AppConfig.DNS_GOOGLE_ADDRESSES
import pey.vpn.net.AppConfig.DNS_GOOGLE_DOMAIN
import pey.vpn.net.AppConfig.DNS_QUAD9_ADDRESSES
import pey.vpn.net.AppConfig.DNS_QUAD9_DOMAIN
import pey.vpn.net.AppConfig.DNS_YANDEX_ADDRESSES
import pey.vpn.net.AppConfig.DNS_YANDEX_DOMAIN
import pey.vpn.net.AppConfig.GEOIP_CN
import pey.vpn.net.AppConfig.GEOSITE_CN
import pey.vpn.net.AppConfig.GEOSITE_PRIVATE
import pey.vpn.net.AppConfig.GOOGLEAPIS_CN_DOMAIN
import pey.vpn.net.AppConfig.GOOGLEAPIS_COM_DOMAIN
import pey.vpn.net.AppConfig.HEADER_TYPE_HTTP
import pey.vpn.net.AppConfig.LOOPBACK
import pey.vpn.net.AppConfig.PROTOCOL_FREEDOM
import pey.vpn.net.AppConfig.TAG_BLOCKED
import pey.vpn.net.AppConfig.TAG_DIRECT
import pey.vpn.net.AppConfig.TAG_FRAGMENT
import pey.vpn.net.AppConfig.TAG_PROXY
import pey.vpn.net.AppConfig.WIREGUARD_LOCAL_ADDRESS_V4
import pey.vpn.net.AppConfig.WIREGUARD_LOCAL_ADDRESS_V6
import pey.vpn.net.dto.EConfigType
import pey.vpn.net.dto.NetworkType
import pey.vpn.net.dto.ProfileItem
import pey.vpn.net.dto.RulesetItem
import pey.vpn.net.dto.V2rayConfig
import pey.vpn.net.dto.V2rayConfig.RoutingBean.RulesBean
import pey.vpn.net.extension.isNotNullEmpty
import pey.vpn.net.fmt.HttpFmt
import pey.vpn.net.fmt.Hysteria2Fmt
import pey.vpn.net.fmt.ShadowsocksFmt
import pey.vpn.net.fmt.SocksFmt
import pey.vpn.net.fmt.TrojanFmt
import pey.vpn.net.fmt.VlessFmt
import pey.vpn.net.fmt.WireguardFmt
import pey.vpn.net.util.JsonUtil
import pey.vpn.net.util.Utils

object V2rayConfigManager {

    fun getV2rayConfig(context: Context, guid: String): pey.vpn.net.dto.ConfigResult {
        try {
            val config = MmkvManager.decodeServerConfig(guid) ?: return pey.vpn.net.dto.ConfigResult(
                false
            )
            if (config.configType == EConfigType.CUSTOM) {
                val raw = MmkvManager.decodeServerRaw(guid) ?: return pey.vpn.net.dto.ConfigResult(
                    false
                )
                val domainPort = config.getServerAddressAndPort()
                return pey.vpn.net.dto.ConfigResult(true, guid, raw, domainPort)
            }
            ...
            e.printStackTrace()
            return pey.vpn.net.dto.ConfigResult(false)
        }
    }

    private fun getV2rayNonCustomConfig(context: Context, config: ProfileItem): pey.vpn.net.dto.ConfigResult {
        val result = pey.vpn.net.dto.ConfigResult(false)
    ...
    fun getProxyOutbound(profileItem: ProfileItem): V2rayConfig.OutboundBean? {
        return when (profileItem.configType) {
            EConfigType.VMESS -> pey.vpn.net.fmt.VmessFmt.toOutbound(profileItem)
